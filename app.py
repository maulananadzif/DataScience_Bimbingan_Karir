...
# Membaca data dari file dengan format khusus dan membentuk DataFrame
with open("data/hungarian.data", encoding='Latin1') as file:
    lines = [line.strip() for line in file]

# Mengorganisir data menjadi baris-baris dan kolom-kolom untuk membentuk DataFrame
...
# Mengatur jenis data kolom menjadi float dan mengganti nilai -9.0 dengan NaN
df = df.iloc[:, :-1]
df = df.drop(df.columns[0], axis=1)
df = df.astype(float)
df.replace(-9.0, np.NaN, inplace=True)

# Memilih subset kolom yang akan digunakan dalam analisis
df_selected = df.iloc[:, [1, 2, 7, 8, 10, 14, 17, 30, 36, 38, 39, 42, 49, 56]]

# Mengonversi nomor kolom menjadi nama fitur sesuai mapping yang diberikan
column_mapping = {
    2: 'age',
    3: 'sex',
    8: 'cp',
...
# Mengisi nilai yang hilang dengan rata-rata masing-masing fitur
...
# Mengisi nilai yang hilang dengan rata-rata yang telah dihitung
df_clean = df_selected.fillna(value=fill_values)
df_clean.drop_duplicates(inplace=True)

# Menyiapkan data dan target untuk oversampling menggunakan SMOTE
...
# Memuat model yang telah di-train sebelumnya
model = pickle.load(open("model/xgb_oversample_model.pkl", 'rb'))

# Melakukan prediksi menggunakan model yang telah di-train
...
with tab2:
    ...
    if file_uploaded:
        ...
        # Mengonversi hasil prediksi ke dalam teks
